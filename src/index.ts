...
async function handleXeroPnlByCode(request: Request, env: Env): Promise<Response> {
  ...
  // activity — that's the whole point of "all cost codes", not just used
  // ones. Other code types (Asset/Liability/Equity — capital purchases,
  // loan repayments, director's loan movements etc.) are NOT zero-filled,
  // since there could be dozens of balance-sheet codes never touched by a
  // real cash transaction — but any of them WITH actual activity this year
  // is included below, because this report tracks real cash movement in a
  // P&L-shaped layout, not strict accrual profit. A vehicle purchase or a
  // loan repayment is genuine cash out even though it's not a P&L expense.
